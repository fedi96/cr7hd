	...
	$.getJSON(nbaAPI, function (json) {
		...
		if (json.gs.mid > 0) {
			...
			$.each(games, function (i, game) {
				...
				var gameLinks = "";
				gameLinks = gameLinks + "<a href='http://rjh.fun/play.html?league=NBA&date="+json.gs.gdte+"&pk="+pk+"&cdn=1' data-ajax='false'><button class='bttn-slant bttn-xs bttn-warning'>HOME</button></a><a href='http://rjh.fun/play.html?league=NBA&date="+json.gs.gdte+"&pk="+pk+"&cdn=4' data-ajax='false'><button class='bttn-slant bttn-xs bttn-warning'>AWAY</button></a>";
