	...
	$.getJSON(nbaAPI, function (json) {
		if (json.gs.mid > 0) {
			...
			$.each(games, function (i, game) {
				...
				var strmv = game.v.tc.toLowerCase()+"-"+game.v.tn.toLowerCase();
				var strmh = game.h.tc.toLowerCase()+"-"+game.h.tn.toLowerCase();
				switch (logoaway) {default:
						break;
					case 'NOP':
						logoaway = "no";
						break;
				...
				var gameLinks = "";
				gameLinks = gameLinks + "<a href='http://rjh.fun/play.html?league=NBA&pk="+pk+"&cdn="+strmh+"' data-ajax='false'><button class='bttn-slant bttn-xs bttn-warning'>HOME</button></a><a href='http://rjh.fun/play.html?league=NBA&pk="+pk+"&cdn="+strmv+"' data-ajax='false'><button class='bttn-slant bttn-xs bttn-warning'>AWAY</button></a>";
