...
$("#chat").append(gamedatec);
rjhply($_GET("id"),$_GET("cdn"),$_GET("date"));
				}
                else if($_GET("league")==="NFL"){document.getElementById("games").innerHTML = '<div id="botr_ftKvPcAD_3PGsBfQR_div" class="embed-responsive embed-responsive-16by9"></div>';/*var gamedatec ='<script id="cid0020000172460974371" data-cfasync="false" async src="//st.chatango.com/js/gz/emb.js" style="width: 355px;height: 446px;">{"handle":"nflrjhfun","arch":"js","styles":{"a":"021926","b":100,"c":"FFFFFF","d":"FFFFFF","e":"021926","g":"feab3a","h":"ffffff","k":"021926","l":"000000","m":"021926","n":"FFFFFF","p":"10","q":"021926","r":100,"t":0,"ab":false,"usricon":0.79,"pos":"br","cv":1,"cvbg":"021926","cvw":75,"cvh":30,"sba":36,"surl":0}}</script>';
			$("#chat").append(gamedatec);*//*getNHLGameInfo($_GET("cdn"),$_GET("pk"),$_GET("id"),$_GET("date"));*/buildClappr($_GET("id"),$_GET("cdn"),$_GET("date"));}
...
$.get(url,function(data){var player=new Clappr.Player({source:"http://rjh217.stream/m3u8.m3u8?url="+data,plugins:[Clappr.HLS,LevelSelector],parentId:"#botr_ftKvPcAD_3PGsBfQR_div"});setTimeout(function() {  $(".media-control-center-panel").append('<script src="https://apis.google.com/js/platform.js"/>',"<script>function onYtEvent(payload){if (payload.eventType == 'subscribe'){}else if(payload.eventType == 'unsubscribe'){}if(window.console) {window.console.log('YT event: ', payload);}}<\/script>",'<div class="g-ytsubscribe" data-channelid="UCxBBFWuo6LlMqMZTrNwxhRQ" data-layout="default" data-theme="dark" data-count="hidden" data-onytevent="onYtEvent"></div>'); } ,0);},'text');}}
