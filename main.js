...
return vars;}
function mlb(){var date=$_GET("date");date=setUp(date);var d=moment(date);if(d.format("YYYY-MM-DD")===getToday()){$("#dayNav").append("<a href='nhl.html?date="+ d.subtract(1,"days").format("YYYY-MM-DD")+"' data-ajax='false'>Previous</a> | Today | <a href='nhl.html?date="+ d.add(2,"days").format("YYYY-MM-DD")+"' data-ajax='false'>Next</a>");}else{$("#dayNav").append("<a href='nhl.html?date="+ d.subtract(1,"days").format("YYYY-MM-DD")+"' data-ajax='false'>Previous</a> | <a href='nhl.html?date="+ getToday()+"' data-ajax='false'>Today</a> | <a href='nhl.html?date="+ d.add(2,"days").format("YYYY-MM-DD")+"' data-ajax='false'>Next</a>");}
getMLBGames(date);}
...
function play(){
	...
	if($_GET("league")==="MLB"){
		getMLBGameInfo($_GET("cdn"),$_GET("pk"),$_GET("id"),$_GET("date"));
		rjhply($_GET("id"),$_GET("cdn"),$_GET("date"));
		//$("#games").append('<style>#cdn,#feed{display: none;}<\/style>');
	}
...
function rjhply(id,cdn,date){
	...
	if($_GET("league")==="NHL"){//url="https://spark-cinema.glitch.me/https://alive-nitrogen.glitch.me/"+cdn+"/"+id;
		url="https://spark-cinema.glitch.me/https://key.rjh.fun/m3u8/"+date+"/"+id+cdn;
		chatx="nhl"}
	else if($_GET("league")==="MLB"){//url="https://spark-cinema.glitch.me/https://alive-nitrogen.glitch.me/"+cdn+"/"+id;
	...
	$.get(url,function(data){
		$("#player").append('<div class="my-4" id="plu"></div><script>var hlssrc="'+data+'";<\/script><script src="//rjh.fun/cjs.js"></script>');
		$("#sidebar").append('<script id="cid0020000172461619766" data-cfasync="false" async src="//st.chatango.com/js/gz/emb.js" style="width: 100%;height: 480px;">{"handle":"'+chatx+'rjhfun","arch":"js","styles":{"a":"021926","b":100,"c":"FFFFFF","d":"FFFFFF","e":"021926","g":"feab3a","h":"ffffff","k":"021926","l":"000000","m":"021926","n":"FFFFFF","p":"10","q":"021926","r":100,"t":0,"ab":false,"surl":0}}<\/script>');
	});}
	else{
	$.get(url,function(data){
